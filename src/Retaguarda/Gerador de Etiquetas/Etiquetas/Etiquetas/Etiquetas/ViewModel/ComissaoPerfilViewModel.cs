...
{
    public class ComissaoPerfilViewModel : ViewModelBase
    {

        #region Variáveis

        public ObservableCollection<ComissaoPerfilDTO> ListaComissaoPerfil { get; set; }
        private ComissaoPerfilDTO _ComissaoPerfilSelected;
        private bool _isEditar { get; set; }

        #endregion

        #region Construtor
        public ComissaoPerfilViewModel()
        {
            try
            {
                ListaComissaoPerfil = new ObservableCollection<ComissaoPerfilDTO>();
                IndiceNavegacao = 0;
        ...
        #region Infra
        public ComissaoPerfilDTO ComissaoPerfilSelected
        {
            get { return _ComissaoPerfilSelected; }
            set
            {
                _ComissaoPerfilSelected = value;
                notifyPropertyChanged("ComissaoPerfilSelected");
            }
        }
        ...
        public void BotaoInserir()
        {
            try
            {
                ComissaoPerfilSelected = new ComissaoPerfilDTO();
                IsEditar = true;
            }
            catch (Exception ex)
            {
        ...
        public void BotaoAlterar()
        {
            try
            {
                if (ComissaoPerfilSelected != null)
                    IsEditar = true;
                else
        ...
        public void BotaoLocalizar()
        {
            try
            {
                AtualizarListaComissaoPerfil(0);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Alerta do sistema", MessageBoxButton.OK, MessageBoxImage.Error);
            ...
            get
            {
                if (BotaoExportarXlsCommand == null)
                {
                    ...
                    (
                        param =>
                        {
                            DataGridExportacao.ItemsSource = ListaComissaoPerfil;
            ...
            get
            {
                if (PagSeguinteCommand == null)
                {
                    ...
                    (
                        param =>
                        {
                            AtualizarListaComissaoPerfil(1);
                        },
                        param =>
                        {
                            return true;
